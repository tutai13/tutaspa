...
namespace API.Controllers
{
	...
	public class ThongKeController : ControllerBase
	{
		...
		public IActionResult GetSanPhamSoLuong()
		{
			...
			var totalSoLuong = chiTietSanPham.Sum(x => x.SoLuong);
			var result = chiTietSanPham
				.Join(_context.Products,
					  ...
					  (g, p) => new ThongKeSanPhamDTO
					  {
						  ...
						  SoLuong = g.SoLuong,
						  PhanTram = totalSoLuong > 0 ? (float)Math.Round((g.SoLuong * 100.0 / totalSoLuong), 2) : 0
					  })
				.ToList();

			return Ok(result.Any() ? result : new List<ThongKeSanPhamDTO>());
		}
		...
		public IActionResult GetDichVuSoLuong()
		{
			...
			var grouped = chiTietDichVu
				...
				.Join(_context.DichVus.AsQueryable(), // Sử dụng IQueryable để tối ưu
					  g => g.Key,
					  p => p.DichVuID,
					  (g, p) => new
					  {
			...
			return Ok(result.Any() ? result : new List<ThongKeDichVuDTO>());
		}
		...
		public async Task<ActionResult<IEnumerable<HoaDon>>> GetHoaDon()
		{
			...
			return Ok(result.Any() ? result : new List<HoaDon>());
		}
